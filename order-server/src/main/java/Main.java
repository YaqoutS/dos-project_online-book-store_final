...
public class Main {
	...
	private static void synchronizeOrderWithSecondary(Order order) {
        // URL of the synchronization API on the secondary server
        String apiUrl = "http://localhost:4566/sync-order";
	...
	public static void main(String[] args) {
		...
		post("/purchase/:bookId", (req, res) -> {
			...
			try {
	            ...
	            synchronizeOrderWithSecondary(order);
		...
		post("/sync-order", (req, res) -> {
            try {
        		...
        		try {
        			...
        			String apiUrl = "http://localhost:4568/dec/" + receivedOrder.getBookID();
    	            URL url = new URL(apiUrl);
    	            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
    	            connection.setRequestMethod("PUT");
    	            ...
    	            System.out.println("Dec from sync-order API / primary server " + response);
    	            reader2.close();	 
                ...
                return "Order synchronized successfully with the primary server.";
